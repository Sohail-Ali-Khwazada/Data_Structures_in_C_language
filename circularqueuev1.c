//IMPLEMENTATION OF CIRCULAR QUEUE -> IF-ELSE APPROACH.
#include<stdio.h>
#include<stdlib.h>
...
char isEmpty(struct queue *pointertoqueue) {
    if (pointertoqueue -> front == -1 && pointertoqueue -> rear == -1) {
        return 1;
    }
    return 0;
}
...
char isFull(struct queue *pointertoqueue) {
    if ((pointertoqueue -> front == 0 && pointertoqueue -> rear == pointertoqueue -> max-1) || (pointertoqueue -> front == pointertoqueue -> rear + 1)) {
        return 1;
    }
    return 0;
}
...
void Enqueue (struct queue *pointertoqueue, struct student *s) {
    if (isEmpty(pointertoqueue) == 1) {
        pointertoqueue -> front = pointertoqueue -> rear = 0;
    } else if (pointertoqueue -> rear == pointertoqueue -> max-1) {
        pointertoqueue -> rear = 0;
    } else {
        (pointertoqueue -> rear)++;
    }
...
struct student dequeue (struct queue *pointertoqueue) {
    struct student temp = (pointertoqueue -> studarray)[pointertoqueue -> front];
    if (pointertoqueue -> front == pointertoqueue -> rear) {
        pointertoqueue -> front = pointertoqueue -> rear = -1;
    } else if (pointertoqueue -> front == pointertoqueue -> max-1) {
...
struct student *peek (struct queue *pointertoqueue,int choice) {
    if (choice == 1) {
        return &(pointertoqueue -> studarray)[pointertoqueue -> front];
    }
